...
class MyPDFHandler(object):
    ...
    def add_one_bookmark(self, title, page, parent=None, color=None):
        '''
        ...
        :return: None
        '''
        # 为了防止乱码，这里对title进行utf-8编码 # .decode('utf-8')
        self.__writeable_pdf.addBookmark(title, page, parent,  color, True, False,"/XYZ", None, None, None)
    ...
    def read_bookmarks_from_txt(self, txt_file_path, page_offset):
        '''
        从文本文件中读取书签列表
        ...
        '''
        bookmarks = []
        with open(txt_file_path, 'r', encoding='utf_8') as fin:
            print(fin.readlines)
            for line in fin:
                line = line.rstrip()
                if not line:
        ...
        nest_bookmarks = []
        k, v = [], []
        for marks in bookmarks:
            if "章" in marks[0]:
                k.append(marks)
                if len(k) == 2:
                    nest_bookmarks.append({k[0]:v})
                    k, v = [k[1]], []
            else:
                v.append(marks)
        return nest_bookmarks

    def add_bookmarks_by_read_txt(self, txt_file_path, page_offset):
        '''
        ...
        for marks in bookmarks:
            for k,v in marks.items():
                parent = self.__writeable_pdf.addBookmark(title=k[0], pagenum=k[1])
                for i in v:
                    self.__writeable_pdf.addBookmark(title=i[0], pagenum=i[1], parent=parent)
